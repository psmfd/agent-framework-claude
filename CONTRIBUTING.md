...
- Lockstep duplication (errors on drift) — the deliberately duplicated `SECRET_PATTERNS` (secrets-guard pair, plus the third copy in `skills/expertise/scripts/expertise-create.sh`; ADR-096), identity-helper functions (`sanitize`, `is_valid_login`, `parse_owner_repo`, `GH_LOGIN_RE`; gh-identity pair), and expertise host-gate predicates (`is_loopback_host`, `is_lima_gateway_host`; expertise script pair) must stay byte-identical across their copies. The extractor errors loudly when a target is not found in its expected shape rather than silently passing. See ADR-083.
...
| `skills/<name>/` (add/remove) | `README.md` (Current Skills + directory tree), `setup.sh` (`CLAUDE_LINKS` `skills:skills`, once), `validate.sh` (`check_symlinks` pair, once) | One H3 entry per skill (not gated by `validate.sh` — hand-verify); bundled `scripts/*.sh` are shellcheck- and bash-3.2-gated automatically |
...
| `rules/expertise-capture.md` (candidate schema) | `skills/expertise/SKILL.md` (create-step `entryType`/`severity` enums) | The rule reuses the enums by citation; the value lists must stay identical (manual — automation tracked in #101) |
